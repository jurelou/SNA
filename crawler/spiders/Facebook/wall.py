...
def extract_post_data(response):
    ...
    comments_likes = response.body.xpath("//div/span/span/a[not(text()='React') and not(text()='Like')]")
    if likes:
...
def extract_posts_data(response):
    ...
    posts = response.body.xpath("//div/div/div/span[starts-with(@id, 'like')]/a[not(text()='React') and not(text()='Like')]")
    if posts:
        for post in posts:
